...
void AdjustBuffer(par_t *par) {
	par->mybufsize = ( ( (par->nrow / par->np) + par->np) * sizeof(MPI_DOUBLE)
			+ MPI_BSEND_OVERHEAD) * 4;	// applicate buffer size large enough. it is reused for all MPI send opreation
