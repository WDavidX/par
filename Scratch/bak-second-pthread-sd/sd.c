...
//	  fprintf(stderr,"Failed to malloc rowhits");
  ...
  blocksize=mat->nrows/NTHREADS;
  lastblocksize=mat->nrows-blocksize*(NTHREADS-1);
  printf("nrows: %3; \t block-size %d; \t lastblock-size %3d\n",mat->nrows, blocksize,lastblocksize);
  ...
  for (i=0; i<mat->nrows; ++i) {
    ...
    rowtothits=0;
    for (blk=0;blk<NTHREADS;++blk){
	  fprintf(stderr,"BLK %d nhits: %d\n",blk ,blknhits[blk]);
	  for (j=0;j<blknhits[blk];++j){
				rowhits[rowtothits] = (threadpack[blk].hits)[j];
				fprintf(stderr, "Sim %5.3f (pack %5.3f)   Val %3d (pack %3d) --> %3d\n",
						rowhits[rowtothits].key, (threadpack[blk].hits)[j].key,
						rowhits[rowtothits].val, (threadpack[blk].hits)[j].val,
						rowhits[rowtothits].val	+ blk_rstart[blk]);
				rowhits[rowtothits].val = rowhits[rowtothits].val
						+ blk_rstart[blk];
//				fprintf(stderr,
//						"rowtothits %d; Merged hits %d: sim %5.3f (%5.3f)  val %d(%d) rst %d\n",
//						rowtothits, j, rowhits[j].key,
//						(threadpack[blk].hits)[j].key, rowhits[j].val,
//						(threadpack[blk].hits)[j].val, blk_rstart[blk]);
				rowtothits += 1;
	  }
    }
    fprintf(stderr,"Row %d num simhits %d\n",i,rowtothits);
    fprintf(stderr,"Total potential hit %d Vs nnbrs %d\n",rowtothits,params->nnbrs);
...
//    j=0;
//    for (int kk=0;kk<mat->nrows;++kk){
////    	fprintf(stderr,"Row: %d; Idx %2d; BLK %2d; hits-key %d; hits-val %5.3f; cand %5.3f; marker %d\n",
////    			i, kk, kk/NTHREADS,hits[kk].key, hits[kk].val, cand[kk], marker[kk]);
//    	fprintf(stderr,"Row: %d; rIdx %2d; BLK %2d(%d);    hits-k-v %5.3f %2d;   cand-k-v %5.3f %2d; marker %d\n",
//    			i, kk, j,blk_rstart[j+1]-blk_rstart[j],
//    			hits[kk].key, hits[kk].val,
//    			cand[kk].key, cand[kk].val,
//    			marker[kk]);
//    	if (kk+1>=blk_rstart[j+1])
//    		++j;
//    }
    fprintf(stderr,"End of row %d final nhits %d\n",i,rownhits_final);
  ...
  fprintf(stderr,"End of all iterations.\n");
  ...
  gk_csr_Free(&mat);
  fprintf(stderr,"sd accomplished.\n");
  return;
}
