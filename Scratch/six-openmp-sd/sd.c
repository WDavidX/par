...
void ComputeNeighbors(params_t *params)
{
  ...
  /* create blocks by average non-zero entry */
  ...
  int chunknum=(mat->nrows-1)/CHUNKSIZE+1;
  printf("Chunksize %d; Chunknum %d\n",CHUNKSIZE,chunknum);
  gk_csr_t** chunks=malloc(sizeof(gk_csr_t*)*chunknum);
  for (i=0;i<chunknum;++i){
  	if (i==chunknum-1){
  		chunks[i]=gk_csr_ExtractSubmatrix(mat,i*CHUNKSIZE,mat->nrows-i*CHUNKSIZE);
  	}else{
  		chunks[i]=gk_csr_ExtractSubmatrix(mat,i*CHUNKSIZE,CHUNKSIZE);
  	}
  	if (!chunks[i]){
  		fprintf(stderr, "Cannot create chunks %i",i);
  		exit(1);
  	}
  	gk_csr_CreateIndex(chunks[i],GK_CSR_COL);
  }
