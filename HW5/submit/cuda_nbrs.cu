...
//	printf("    timer3:  CPHD   (sec):   %.2lf\n", params->timer_3);
	...
	sim_t *dout;
	sim_t *houtsave;
	int qblkidx, dblkidx,  houtnum = params->ndrows + params->nnbrs;
	...
	for (qblkidx = 0; qblkidx < params->nqrows; qblkidx += 1)
		hout[qblkidx] = &houtsave[qblkidx * houtnum];
	...
	for (qID = params->startid; qID < params->endid; qID += params->nqrows) {
		...
		//printf(" Working on query block %d/%d (%.2f%) nqrows %d\n", qID, ndocs,(100.0*qID)/ ndocs, nqrows);
		...
		for (dID = 0; dID < vault->ndocs; dID += params->ndrows) {
			...
			gk_startwctimer(params->timer_3);
			/* CUDA kernel call for similarity calculation */
			Sim<<<nqrows,params->ndrows>>> 		(dcolind,dcolval,drowptr,qID,dID,params->nqrows,ndrows,dout);
			gk_stopwctimer(params->timer_3);
